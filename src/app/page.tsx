import {GetCircuits, GetErgastRaces, GetMeetings} from "@/lib/actions/actions"
import {getNextMeeting} from "@/lib/utils"
import HomePage from "./components/HomePage"

export default async function Render() {
    try {
        const races = await GetErgastRaces()
        ...
        return <HomePage nextMeeting={nextMeeting} timeLeft={timeLeft} track={findTrackMeeting} nextRace={nextRace} />
    } catch (error) {
        return(
            <div className="text-gray-950 w-full h-full items-center flex justify-center">
                Render error: {JSON.stringify(error)}
            </div>
        )
    }
}
